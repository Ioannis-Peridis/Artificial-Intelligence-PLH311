...
# Remove dependency from commonroad_dc module
...
class SearchBaseClass(ABC):
    ...
    def __init__(self, scenario: Scenario, planningProblem: PlanningProblem, automaton: ManeuverAutomaton,
        ...
        self.obstacles = self.extract_collision_obstacles_boundaries(print_flag=False)
        self.boundaries = self.extract_outer_lanelet_bounds(print_flag=True)
        # for obstacle in self.obstacles:
                # print("Returned obstacle: ", obstacle)
        # print("Boundaries y coordinates are: ", self.boundaries)

    def extract_outer_lanelet_bounds(self, print_flag = False, outer_only = True):
        """
        extra code for reverse engineering, by IC
        returns a np array with the outer y coordinate boundaries of the road, [y_upper/left, y_lower/right]
        """
        if outer_only:
                lanelets = self.scenario.lanelet_network.lanelets
                left_bound = lanelets[-1].left_vertices[0]
                right_bound = lanelets[0].right_vertices[0]
                boundaries = np.array([left_bound[1], right_bound[1]])
                return boundaries
        # for lanelet in self.scenario.lanelet_network.lanelets:
        #         print("Lanelet network consists of: ", lanelet.left_vertices)

    def extract_collision_obstacles_boundaries(self, print_flag = False):
        """
        extra code for reverse engineering, by IC
        returns a np array with obstacle objects, each object is structed [x_center_position, y_center_position, length, width]
        """
        obstacles = []
        scenario_obstacles_values = list(self.scenario._static_obstacles.values())
        scenario_obstacles_list = self.scenario._static_obstacles
        if not print_flag:
                for scenario_obj in scenario_obstacles_list:
                        obstacle_position = scenario_obstacles_list[scenario_obj].initial_state.position
                        obstacle_length = scenario_obstacles_list[scenario_obj].obstacle_shape.length
                        obstacle_width = scenario_obstacles_list[scenario_obj].obstacle_shape.width
                        obstacles.append(np.append(obstacle_position, (obstacle_length, obstacle_width)))
                return obstacles

        # print("scenario info: ", self.scenario)
        if scenario_obstacles_values is None:
                print("Obstacles are empty")
        else:
                print("Obstacles exist")
                print(scenario_obstacles_values)
                print(scenario_obstacles_list)
                for scenario_obj in scenario_obstacles_list:
                        if scenario_obj is not None:  
                                ...
                                obstacle_width = scenario_obstacles_list[scenario_obj].obstacle_shape.width
                                obstacles.append(np.append(obstacle_position, (obstacle_length, obstacle_width)))
                                print("Object id: ", scenario_obj, ", position: ", obstacle_position, ", length: ", obstacle_length, ", width: ", obstacle_width)
        return obstacles


    def extract_collision_obstacles_information(self, print_flag = False):
        """
        extra code for reverse engineering, by IC
        returns a list with obstacle objects, each object is structed [x_center_position, y_center_position, length, width]
        """

        return [elem.tolist() for elem in self.extract_collision_obstacles_boundaries()]
    ...
    def parse_planning_problem(self) -> None:
        ...
        if hasattr(self.planningProblem.goal.state_list[0], 'position'):
            if hasattr(self.planningProblem.goal.state_list[0].position, 'vertices'):
                ...
                # print("HAS VERTICES ", self.position_desired[0].start, "\t",self.position_desired[0].end,"\t",self.position_desired[1].start, "\t",self.position_desired[1].end)
    ...
    def plot_solution(self, path_solution, node_status, list_states_nodes, time_pause):
        node_status = update_visualization(primitive=path_solution[-1], status=MotionPrimitiveStatus.SOLUTION,
                                           ...
                                           count=len(list_states_nodes), time_pause=time_pause)
        ...
        for prim in path_solution:
            node_status = update_visualization(primitive=prim, status=MotionPrimitiveStatus.SOLUTION,
                                               dict_node_status=node_status, path_fig=self.goal_path_fig,
                                               config=self.config_plot,
                                               count=len(list_states_nodes), time_pause=time_pause)
        list_states_nodes.append(copy.copy(node_status))
        return list_states_nodes

    def plot_colliding_primitives(self, current_node: Union[Node, CostNode], path_translated, node_status,
                                  list_states_nodes, time_pause, visited_nodes):
        ...
        return list_states_nodes, node_status, visited_nodes
    ...
    def calc_euclidean_distance(self, current_node: CostNode) -> float:
        """
    ...
    def reached_goal(self, path: List[State]) -> bool:
        ...
        """
        goal = self.planningProblem.goal.state_list[0].position
        for state in path:
            goal_left = goal.center[0] - goal.length/2
            goal_right = goal.center[0] + goal.length/2
            goal_lower = goal.center[1] - goal.width/2
            goal_upper = goal.center[1] + goal.width/2
            if goal_left <= state.position[0] <= goal_right and goal_lower <= state.position[1] <= goal_upper:
                return  True
        return False
        # for i in range(len(path)):
    ...
    def extract_goal_information(self):
        """ 
        ...
        return [goal.center[0], goal.center[1], goal.length, goal.width]
    ...
    def is_collision_free(self, path: List[State]) -> bool:
        ...
        # prints for reverse engineering IC
        # print("timestep: ", trajectory.initial_time_step, " ...state list: ", trajectory.state_list)
        for state in trajectory.state_list:
                position = state.position
                # print("State position: ", state.position)
                # boundary check
                if position[1] > self.boundaries[0] or position[1]< self.boundaries[1] or position[0]>100:
                        # print("Found boundary!")
                        return False
                for obstacle in self.obstacles:
                        x_start = obstacle[0] - obstacle[2]/2
                        x_end = obstacle[0] + obstacle[2]/2
                        y_start = obstacle[1] - obstacle[3]/2
                        y_end = obstacle[1] + obstacle[3]/2
        ...
        # print("trajectory prediction is: ", traj_pred)
